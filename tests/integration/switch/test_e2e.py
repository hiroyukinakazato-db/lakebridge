"""End-to-end tests for Switch transpiler complete lifecycle

Tests the full workflow from installation to uninstallation.
Requires actual Databricks workspace and can use TestPyPI or PyPI.

Environment variables:
- LAKEBRIDGE_SWITCH_E2E: Set to "true" to enable these tests
- LAKEBRIDGE_SWITCH_PYPI_SOURCE: "testpypi" (default) or "pypi"
- LAKEBRIDGE_SWITCH_INCLUDE_SYNC: Set to "true" to include sync execution test
- LAKEBRIDGE_SWITCH_KEEP_RESOURCES: Set to "true" to keep resources after test (for debugging)
- DATABRICKS_HOST: Databricks workspace URL
- DATABRICKS_TOKEN: Authentication token

Usage examples:
    # Basic E2E test (async only)
    LAKEBRIDGE_SWITCH_E2E=true pytest tests/integration/switch/test_e2e.py::TestSwitchE2E::test_complete_lifecycle -v

    # Include synchronous execution test (takes ~10 minutes)
    LAKEBRIDGE_SWITCH_E2E=true LAKEBRIDGE_SWITCH_INCLUDE_SYNC=true pytest tests/integration/switch/test_e2e.py -v

    # Use production PyPI instead of TestPyPI
    LAKEBRIDGE_SWITCH_E2E=true LAKEBRIDGE_SWITCH_PYPI_SOURCE=pypi pytest tests/integration/switch/test_e2e.py -v

    # Debug mode (keep resources after test)
    LAKEBRIDGE_SWITCH_E2E=true LAKEBRIDGE_SWITCH_KEEP_RESOURCES=true pytest tests/integration/switch/test_e2e.py -v

    # Run all Switch tests except E2E
    pytest tests/integration/switch/ -v
"""
import json
import logging
import os
...
class TestSwitchE2E:
    """Complete end-to-end tests for Switch lifecycle"""
    ...
    def test_complete_lifecycle(self, workspace_client, pypi_source):
        """Test: Install → Async Transpile → Reinstall → Uninstall"""
        ...
        try:
            ...
            # Check if Switch package is available
            try:
                import switch
    ...
    def test_sync_transpilation(self, workspace_client):
        """Test synchronous transpilation with wait_for_completion"""
    ...
    def _install_switch(self, pypi_source):
        ...
        if pypi_source == "pypi":
            ...
            # For TestPyPI, need to handle specially
    ...
    def _run_transpile(self, source_dialect, input_source, output_folder):
        ...
        raise ValueError("No JSON output found in transpile command")
