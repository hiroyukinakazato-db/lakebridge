...
Switch supports multiple source formats with flexible preprocessing:

### SQL Sources (Default)
- **Preprocessing**: Automatic SQL comment removal and whitespace normalization
- **Use Cases**: Traditional SQL migration from various dialects
- **Parameter**: `--source-format sql` (default)

### Generic Sources
- **Preprocessing**: None - preserves original content
- **Use Cases**: XML workflows, configuration files, code in other languages
- **Parameter**: `--source-format generic`
- **Examples**: Informatica XML to Databricks Jobs YAML, Python scripts to notebooks, Airflow DAGs to Databricks Jobs
...
Switch provides built-in sample conversion prompts for major SQL dialects, with extensibility for custom dialects through YAML configuration.
...
### Example: Generic File Conversions

Beyond SQL, Switch supports converting various source formats to different output types:

```bash
# Example 1: Python scripts to notebooks
databricks labs lakebridge transpile \
  --transpiler-config-path ~/.databricks/labs/remorph-transpilers/switch/lib/config.yml \
  --builtin-prompt python \
  --source-format generic \
  --target-type notebook \
  --input-source "/Workspace/python_scripts" \
  --output-folder "/Workspace/notebooks" \
  --catalog-name your_catalog \
  --schema-name your_schema \
  --output json

# Example 2: Airflow DAGs to Databricks Jobs YAML
databricks labs lakebridge transpile \
  --transpiler-config-path ~/.databricks/labs/remorph-transpilers/switch/lib/config.yml \
  --builtin-prompt airflow \
  --source-format generic \
  --target-type file \
  --output-extension .yml \
  --input-source "/Workspace/airflow_dags" \
  --output-folder "/Workspace/databricks_jobs" \
  --catalog-name your_catalog \
  --schema-name your_schema \
  --output json

# Example 3: SQL to YAML workflows
databricks labs lakebridge transpile \
  --transpiler-config-path ~/.databricks/labs/remorph-transpilers/switch/lib/config.yml \
  --builtin-prompt snowflake \
  --source-format sql \
  --target-type file \
  --output-extension .yml \
  --input-source "/Workspace/sql/procedures" \
  --output-folder "/Workspace/workflows" \
  --catalog-name your_catalog \
  --schema-name your_schema \
  --output json
```
...
- `--builtin-prompt` - Built-in conversion prompt to use. Options include SQL dialects (`mysql`, `snowflake`, `oracle`, `postgresql`, `redshift`, `teradata`, `netezza`, `tsql`), code conversions (`python`, `scala`), and workflow conversions (`airflow`)
...
#### Optional Parameters
- `--source-format` - Source file format type. Choose from: `sql`, `generic` (default: `sql`)
- `--target-type` - Output format type. Choose from: `notebook`, `file` (default: `notebook`)
- `--output-extension` - File extension when using `--target-type file` (e.g., `.yml`, `.json`)
