...
Use the returned `run_url` to monitor the conversion progress and view detailed results in your Databricks workspace.
...
- `--input-source` - Databricks workspace path(s) containing SQL files to convert (see Input Source Patterns below)
- `--output-folder` - Databricks workspace path for generated notebooks
- `--source-dialect` - SQL dialect. Choose from: `mysql`, `netezza`, `oracle`, `postgresql`, `redshift`, `snowflake`, `teradata`, `tsql`
