...
- Provides unified entry point for both notebook and file conversion workflows
...
```mermaid
flowchart TD
    main["00_main (Entry Point)"] --> decision{target_type?}
    
    decision -->|notebook| notebookOrch["orchestrate_to_notebook (Full Processing Pipeline)"]
    decision -->|file| fileOrch["orchestrate_to_file (Simplified Processing Pipeline)"]
    
    notebookOrch --> notebookFlow["7-Step Workflow: analyze → convert → validate → fix → split → export → sql_convert(optional)"]
    fileOrch --> fileFlow["3-Step Workflow: analyze → convert → export"]
    
    notebookFlow --> notebookOutput[Python/SQL Notebooks]
    fileFlow --> fileOutput["Generic Files (YAML, JSON, etc.)"]

    %% Styling
    classDef entry fill:#FFE6CC,stroke:#333,stroke-width:3px;
    classDef decision fill:#FFD700,stroke:#333,stroke-width:2px;
    classDef orchestrator fill:#E6F3FF,stroke:#333,stroke-width:2px;
    classDef workflow fill:#F0F8FF,stroke:#333,stroke-width:1px,stroke-dasharray: 3 3;
    classDef output fill:#E0F0E0,stroke:#333,stroke-width:2px;
    
    class main entry;
    class decision decision;
    class notebookOrch,fileOrch orchestrator;
    class notebookFlow,fileFlow workflow;
    class notebookOutput,fileOutput output;
```
...
```mermaid
flowchart TD
    orchestrator[orchestrate_to_notebook] -->|sequentially calls| processing
    
    subgraph processing ["Notebook Processing Workflow"]
        direction TB
        analyze[analyze_input_files] --> convert[convert_with_llm]
        convert --> validate[validate_python_notebook]
        validate --> fix[fix_syntax_with_llm]
        fix --> split[split_code_into_cells]
        split --> export[export_to_notebook]
    ...
    export --> notebooks[Python Notebooks]
    ...
    classDef optional fill:#F0F0F0,stroke:#666,stroke-width:1px,stroke-dasharray: 5 5;
    
    class orchestrator orchestrator;
    class analyze,convert,validate,fix,split,export process;
    class sqlExport optional;
    class table,notebooks data;
    class sqlNotebooks optional;
...
```mermaid
flowchart TD
    orchestrator[orchestrate_to_file] -->|sequentially calls| processing
    
    subgraph processing ["File Processing Workflow"]
        direction TB
        analyze[analyze_input_files] --> convert[convert_with_llm]
        convert --> export[export_to_file]
    end
    ...
    export --> files["Generic Files (YAML, JSON, XML, etc.)"]
    ...
    class orchestrator orchestrator;
    class analyze,convert,export process;
    class table,files data;
    class endpoint external;
```
...
#### analyze_input_files
Scans the input directory for files and performs initial analysis. For SQL sources, it removes comments and normalizes whitespace to get accurate token counts using model-specific tokenizers (Claude uses ~3.4 characters per token, OpenAI uses tiktoken). Files exceeding the `token_count_threshold` are excluded from conversion. All file metadata and analysis results are stored in a timestamped Delta table.

#### convert_with_llm  
Loads conversion prompts (built-in or custom YAML) and sends file content to the configured model serving endpoint. Multiple files are processed concurrently (configurable, default: 4) for efficiency. The LLM transforms source code into Python notebooks with `spark.sql()` calls, preserving business logic while adapting to Databricks patterns.
...
Transforms raw converted code into well-structured notebook cells. Analyzes code flow and dependencies, splitting content at logical boundaries like imports, function definitions, and major operations. Adds appropriate markdown cells for documentation and readability.
...
**How it works:**
- The `analyze_input_files` processor analyzes each file and counts tokens (after removing comments and compressing whitespace for SQL sources)
...
**Causes**: 
- Content exceeds 10MB size limit (notebooks)
