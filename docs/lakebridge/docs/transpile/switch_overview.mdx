...
After executing the transpile command, Switch runs **asynchronously by default** and returns job execution details. Use `--output json` for structured output:
...
Access the `run_url` to monitor job progress in your Databricks workspace. Without `--output json`, results are displayed in table format.
