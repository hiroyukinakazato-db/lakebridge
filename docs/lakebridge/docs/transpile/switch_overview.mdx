...
```mermaid
flowchart TD
    main[00_main] -->|target_type=notebook| orchestrator[orchestrate_to_notebook]
    
    orchestrator -->|sequentially calls| processing
...
The orchestrator executes each processing notebook sequentially using `dbutils.notebook.run()`. The following sections describe each processing step in the workflow:
