...
Switch leverages Large Language Models (LLMs) to convert SQL and other source formats into Databricks-compatible outputs. Using [Mosaic AI Model Serving](https://docs.databricks.com/aws/en/machine-learning/model-serving/), Switch understands code intent and semantics to generate Python notebooks with Spark SQL or generic text files.

Key characteristics:
- **LLM-powered conversion** - Uses Databricks Model Serving endpoints to interpret SQL, workflows, and other formats
- **Flexible outputs** - Python notebooks (.py) by default; generic files (YAML, JSON, etc.) as option
- **Batch processing** - Executes as Databricks Jobs with parallel processing
- **Extensible prompts** - Custom YAML prompts for new dialects and conversion requirements

Switch excels at complex stored procedures and business logic where context matters. Generated outputs provide a strong foundation for migration, though manual review is recommended.
